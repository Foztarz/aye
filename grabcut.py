...
def start(filename):
    ...
    if os.path.isfile(filename + MASK_SUFFIX):
        mask2 = cv2.imread(filename + MASK_SUFFIX, 0)
        print("Loaded existing mask.")
    else:
        mask2 = np.zeros(img.shape[:2],dtype = np.uint8) # mask initialized to PR_BG
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
            ...
            cv2.imwrite(filename+MASK_SUFFIX, mask2)
